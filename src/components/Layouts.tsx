...
const SidebarContent: React.FC<SidebarContentProps> = ({ items, logoUrl, title, collapsed, onClose }) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className={cn('flex items-center border-b border-slate-100 flex-shrink-0',
        ...
        {!collapsed && <span className="text-base font-bold text-primary truncate leading-tight">{title}</span>}
        {onClose && (
          ...
          return (
            <Link key={item.path} to={item.path} title={collapsed ? item.label : undefined}
              ...
              {item.badge && !collapsed && (
                <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full uppercase bg-red-500 text-white">
                  {item.badge}
                </span>
              )}
              {item.badge && collapsed && (
      ...
      <div className="border-t border-slate-100 p-2 flex-shrink-0">
        <button onClick={() => auth.signOut()} title={collapsed ? 'Sair' : undefined}
          className={cn('w-full flex items-center rounded-xl text-sm font-medium text-slate-500 hover:bg-red-50 hover:text-red-500 transition-colors',
            collapsed ? 'justify-center p-3' : 'gap-3 px-3 py-2.5')}>
...
const Sidebar: React.FC<SidebarProps> = ({ items, logoUrl, title, onWidthChange }) => {
  ...
  useEffect(() => { onWidthChange(desktopCollapsed ? W_COLLAPSED : W_OPEN); }, [desktopCollapsed]);

  return (
    <>
      {/* Desktop */}
      <aside className="hidden md:flex flex-col fixed left-0 top-0 h-full bg-white border-r border-slate-200 z-30 overflow-hidden"
        style={{ width: desktopCollapsed ? W_COLLAPSED : W_OPEN, transition: 'width 280ms cubic-bezier(0.4,0,0.2,1)' }}>
        <SidebarContent items={items} logoUrl={logoUrl} title={title} collapsed={desktopCollapsed} />
        <button
      ...
      {/* Mobile topbar */}
      <header className="md:hidden fixed top-0 left-0 right-0 z-40 flex items-center gap-3 bg-white border-b border-slate-200 shadow-sm"
      ...
      {/* Mobile overlay */}
      <div className="md:hidden fixed inset-0 z-40 bg-black/50 backdrop-blur-[2px] transition-opacity duration-300"
        style={{ opacity: mobileOpen ? 1 : 0, pointerEvents: mobileOpen ? 'auto' : 'none' }}
        onClick={() => setMobileOpen(false)} />

      {/* Mobile drawer */}
      <aside className="md:hidden fixed top-0 left-0 h-full bg-white z-50 shadow-2xl"
        style={{ width: W_OPEN, transform: mobileOpen ? 'translateX(0)' : `translateX(-${W_OPEN}px)`, transition: 'transform 280ms cubic-bezier(0.4,0,0.2,1)' }}>
        <SidebarContent items={items} logoUrl={logoUrl} title={title} collapsed={false} onClose={() => setMobileOpen(false)} />
      </aside>
    </>
  );
};
...
export const ClientLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <Sidebar items={navItems} logoUrl={logoUrl} title="GigaNet" onWidthChange={setSidebarW} />
      <main className="min-h-screen transition-all duration-300" style={{ paddingTop: TOPBAR_H }}>
        <div className="md:transition-all md:duration-[280ms]"
          ref={el => {
            if (el) {
              const apply = () => { el.style.marginLeft = window.innerWidth >= 768 ? `${sidebarW}px` : '0'; };
              apply();
...
export const AdminLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems: NavItem[] = [
    ...
    { icon: Bell,            label: 'Notificações',   path: '/admin/notificacoes', badge: 'NEW' },
    { icon: Settings,        label: 'Configurações',  path: '/admin/settings'       },
  ];

  return (
    <div className="min-h-screen bg-slate-50">
      <Sidebar items={navItems} logoUrl={logoUrl} title="Admin" onWidthChange={setSidebarW} />
      <main className="min-h-screen" style={{ paddingTop: TOPBAR_H }}>
        <div className="md:transition-all md:duration-[280ms]"
          ref={el => {
            if (el) {
              const apply = () => { el.style.marginLeft = window.innerWidth >= 768 ? `${sidebarW}px` : '0'; };
